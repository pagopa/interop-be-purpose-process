...
import it.pagopa.pdnd.interop.uservice.purposemanagement.client.model.{VerifiedAttribute, Purpose => ClientPurpose}
...
trait SpecHelper {
  ...
  final lazy val url: String =
    s"http://localhost:8088/pdnd-interop-uservice-purpose-process/${buildinfo.BuildInfo.interfaceVersion}"
  ...
  System.setProperty("PURPOSE_MANAGEMENT_URL", "http://localhost/")
  ...
  object TestDataOne {
    ...
    val purpose: ClientPurpose = ClientPurpose(
      id = id,
      eserviceId = eserviceId,
      descriptorId = descriptorId,
      producerId = producerId,
      consumerId = UUID.fromString(Common.consumerId),
      state = PurposeManagementDependency.PurposeState.ACTIVE,
  ...
  object TestDataFive {
    ...
    val purpose: ClientPurpose = ClientPurpose(
      ...
      producerId = producerId,
      consumerId = UUID.fromString(Common.consumerId),
      state = PurposeManagementDependency.PurposeState.PENDING,
      verifiedAttributes = Seq(
        VerifiedAttribute(
          id = UUID.fromString(Common.verifiedAttributeId1),
          verified = Some(true),
  ...
  object TestDataSix {
    ...
    val purpose: ClientPurpose = ClientPurpose(
      ...
      producerId = producerId,
      consumerId = UUID.fromString(Common.consumerId),
      state = PurposeManagementDependency.PurposeState.SUSPENDED,
  ...
  val purposesAllTrue: Seq[ClientPurpose]        = Seq(TestDataOne.purpose, TestDataTwo.purpose)
  val purposesAllFalse: Seq[ClientPurpose]       = Seq(TestDataThree.purpose)
  val purposesSameTrueFalse: Seq[ClientPurpose]  = Seq(TestDataOne.purpose, TestDataThree.purpose)
  val purposesExcludingFalse: Seq[ClientPurpose] = Seq(TestDataOne.purpose, TestDataFour.purpose)
