package it.pagopa.interop.purposeprocess.api.converters.purposemanagement

import akka.http.scaladsl.marshallers.sprayjson.SprayJsonSupport
import it.pagopa.interop.purposemanagement.client.model.{Problem => PurposeProblem, ProblemError => PurposeProblemError}
import it.pagopa.interop.purposeprocess.model.{Problem, ProblemError}
import spray.json._

import scala.util.Try

object ProblemConverter extends SprayJsonSupport with DefaultJsonProtocol {
  implicit def problemErrorFormat: RootJsonFormat[PurposeProblemError] = jsonFormat2(PurposeProblemError)

  implicit def problemFormat: RootJsonFormat[PurposeProblem] = jsonFormat5(PurposeProblem)

  def dependencyToApi(problem: PurposeProblem): Problem = Problem(
    `type` = problem.`type`,
    status = problem.status,
    title = problem.title,
    detail = problem.detail,
    errors = problem.errors.map(problemErrorToApi)
  )

  def fromString(body: String): Try[Problem] =
    Try(body.parseJson.convertTo[PurposeProblem]).map(problem =>
      Problem(
        `type` = problem.`type`,
        status = problem.status,
        title = problem.title,
        detail = problem.detail,
        errors = problem.errors.map(problemErrorToApi)
      )
    )

  def problemErrorToApi(problemError: PurposeProblemError): ProblemError =
    ProblemError(code = problemError.code, detail = problemError.detail)
}
