...
final case class MissingTenantKindConfiguration(kind: PersistentTenantKind)    extends RiskAnalysisValidationError {
  val message: String = s"Unexpected tenant kind $kind"
}
final case class NoTemplateVersionFound(kind: PersistentTenantKind)            extends RiskAnalysisValidationError {
